-- =========================================================================
-- catch_up.sql — bring a live database up to the latest schema in one paste.
--
-- Safe to run anytime: every statement is "add column if not exists" or
-- "create or replace", so it does nothing where you're already current and
-- fills in whatever's missing. Run this in the Supabase SQL editor if screens
-- come back empty because columns/views are behind the app code.
--
-- (This does NOT touch your data — no deletes, no resets.)
-- =========================================================================

-- ---- Columns added across migrations 0003-0014 --------------------------
...
update trucks t set code = 'T-' || lpad(o.rn::text, 5, '0') from ordered o where o.id = t.id;

-- ---- Refresh all reporting views to their latest form --------------------
...
create or replace view daily_shop_log as
  ...
  from complaints c join trucks t on t.id = c.truck_id where c.voided = false
union all
  select w.done_at::date, w.done_at, 'work_order_done', t.plate, w.code, s.label, m.name
  from work_orders w join complaints c on c.id = w.complaint_id and c.voided = false
  join trucks t on t.id = c.truck_id
  left join specialties s on s.id = w.required_specialty_id
  left join mechanics m   on m.id = w.assigned_mechanic_id
